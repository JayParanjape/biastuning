...
def main():
    ...
    for img_name in sorted(os.listdir(args.data_folder)):
        ...
        img, _ = data_transform(img, label, is_train=False, apply_norm=True)
        ...
        masks = model.get_masks_for_multiple_labels(img_embeds_repeated, x_text)
        ...
        plt.savefig(os.path.join(args.save_path,'preds', img_name))
        plt.close()
